...
mysql-dump:
	...
	@docker exec $(shell docker-compose ps -q mysql) mysqldump $(BACKUP_DB) -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" > $(MYSQL_DUMPS_DIR)/$(BACKUP_DB).sql 2>/dev/null
	@make resetOwner

mysql-restore:
	@docker exec -i $(shell docker-compose ps -q mysql) mysql -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" $(BACKUP_DB) < $(MYSQL_DUMPS_DIR)/$(BACKUP_DB).sql 2>/dev/null
